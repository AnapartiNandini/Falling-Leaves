import greenfoot.*;  // (World, Actor, GreenfootImage, Greenfoot and MouseInfo)

/**
 * Write a description of class FallWorld here.
 * 
 * @author (your name) 
 * @version (a version number or a date)
 */
public class AutumnWorld  extends World
{
    private Scoreboard score;
    
    /**
     * Constructor for objects of class FallWorld.
     * 
     */
    public AutumnWorld()
    {    
        // Create a new world with 20x20 cells with a cell size of 10x10 pixels.
        super(750, 500, 1);
        setPaintOrder(Catcher.class, Leaf.class);
        addObject(new Catcher(), getWidth()/2, getWidth()/2 -25);
        score = new Scoreboard();
        addObject(score,75,25);
    }
    
   /**
    * act - things you want to do each time around go in here
    * 
    */
   public void act()
   {
        addLeaves();
   }
    
   private void addLeaves()
   {       
        //If the next random number generated is less than 1(the statement will only be true)
       if(Greenfoot. getRandomNumber(99) == 1)
       {
            addObject(new Leaf(Greenfoot.getRandomNumber(4)), Greenfoot.getRandomNumber(getWidth()), 0);            
       }
   }
   
   public void update()
   {
        score.addToScore();
        
   }
}
